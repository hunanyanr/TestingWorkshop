...
class DefaultGetPostsRepositoryTest : BehaviorSpec({
    ...
    Given("DefaultGetPostsRepository") {
        val sut = DefaultGetPostsRepository(
            networkDataSource,
            storeDataSource
        )
        When("The Network data source successfully returns posts") {
            ...
            And("Repository is invoked") {
                ...
                Then("It returns either Right or Left") {
                    assert(result.isLeft() || result.isRight())
                }
                Then("Repository returns the network result") {
                    coVerify(exactly = 1) {
                ...
                Then("Repository stores the result data into the cache") {
        ...
        When("The Network data source fails for any reason") {
            val resultList = Arb.list(Arb.post()).next()
            ...
            And("Repository is invoked") {
                ...
                Then("Repository fetches the data from cache") {
